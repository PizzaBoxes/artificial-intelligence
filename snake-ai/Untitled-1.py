...
class SnakeNN:
    ...
    def generate_observation(self, snake):
        #sees which way snake is facing (3 inputs)
        snake_direction = self.get_snake_direction_vector(snake) #which direction is it facing
        barrier_left = self.is_direction_blocked(snake, self.turn_vector_to_the_left(snake_direction)) #is there a barrier to the left
        barrier_front = self.is_direction_blocked(snake, snake_direction) #is there a barrier in front
        barrier_right = self.is_direction_blocked(snake, self.turn_vector_to_the_right(snake_direction)) #is there a barrier to the right
        return np.array([int(barrier_left), int(barrier_front), int(barrier_right)]) #return the inputs
    ...
    def turn_vector_to_the_left(self, vector): #turning
        return np.array([-vector[1], vector[0]])

    def turn_vector_to_the_right(self, vector): #turning
        return np.array([vector[1], -vector[0]])
    ...
    def train_model(self, training_data, model): #gets training data
    ...
    def test_model(self, model): #testing
        steps_arr = []
